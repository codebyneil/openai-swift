...
class LiveTestRecipe: SwiftDataStructuredOutput {
    ...
    static func generateStructuredOutput() -> [String: Any] {
        return [
            ...
            "required": ["title", "servings", "prepTime", "ingredients", "instructions", "difficulty"]
        ]
    }
...
class Event {
    ...
    init(name: String = "", startDate: String = "", endDate: String = "", location: String = "", attendeeCount: Int = 0) {
        self.name = name
...
struct SwiftDataStructuredOutputLiveTests {
    ...
    func testSimpleSwiftDataModel() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .system,
                content: .text("You are a product data generator. Always respond with JSON containing name (string), price (number with decimals), inStock (boolean), category (optional string), and description (optional string).")
            ),
            ChatMessage(
                role: .user,
        ...
        if let name = productData["name"] as? String {
            #expect(name.lowercased().contains("laptop") || name.lowercased().contains("computer"))
        }
    }
    ...
    func testSwiftDataModelWithCustomSchema() async throws {
        ...
        if let ingredients = recipeData["ingredients"] as? [String] {
            ...
            #expect(ingredients.contains { $0.lowercased().contains("chocolate") || $0.lowercased().contains("chip") })
        }
    ...
    func testArrayOfSwiftDataModels() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Generate a JSON object with a 'products' array containing 3 different electronic products. Each product should have name (string), price (number), inStock (boolean), and category (string).")
            )
        ]
    ...
    func testSwiftDataModelWithDates() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Create an event JSON for a tech conference happening next month. Include name, startDate (ISO8601), endDate (ISO8601), location, and attendeeCount (integer).")
            )
        ]
    ...
    func testSwiftDataModelWithEnum() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Create a task JSON with title (string), status (one of: pending, active, completed, cancelled), and priority (integer 1-5). Make it a completed high-priority task.")
            )
        ]
