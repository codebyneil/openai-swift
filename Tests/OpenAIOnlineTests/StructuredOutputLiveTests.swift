...
struct AnalysisResult: StructuredOutputConvertible, Decodable {
    ...
    static var structuredOutputDescription: String? { "Text analysis result with sentiment and keywords" }
...
struct StructuredOutputLiveTests {
    ...
    func testSimpleDecodableResponse() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .system,
                content: .text("You are a weather API. Always respond with JSON containing temperature (number), condition (string), and humidity (integer percentage).")
            ),
            ChatMessage(
                role: .user,
    ...
    func testStructuredOutputConvertibleResponse() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Analyze the sentiment of this text: 'I absolutely love this new feature! It's amazing and works perfectly.'")
            )
        ]
        ...
        #expect(analysis.keywords.contains { $0.lowercased().contains("love") || $0.lowercased().contains("amazing") })
    }
    ...
    func testNestedStructureResponse() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .system,
                content: .text("You are a data generator. Always respond with JSON containing person (name, age, optional email), address (street, city, country), and tags array.")
            ),
            ChatMessage(
                role: .user,
                content: .text("Generate sample data for a software developer living in Silicon Valley.")
            )
        ]
    ...
    func testStructuredChatRequestBuilder() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Is the Earth flat? Answer with a JSON object containing answer (yes/no), explanation (string), and isCorrect (boolean).")
            )
        ]
    ...
    @Test("Streaming with structured output", .disabled("Structured output not supported with streaming"))
    ...
    func testArrayResponseType() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Generate a JSON object with an 'items' array containing 3 objects, each with an id (integer) and name (string) field. Make them programming languages.")
            )
        ]
    ...
    func testEnumResponse() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Return a JSON with status (one of: pending, processing, completed, failed) and message. The task is completed successfully.")
            )
        ]
    ...
    func testDateResponse() async throws {
        ...
        let messages = [
            ChatMessage(
                role: .user,
                content: .text("Create an event JSON with name (string), date (ISO 8601 string like '2024-01-15T10:30:00Z'), and duration in minutes (integer). Make it a tech conference.")
            )
        ]
